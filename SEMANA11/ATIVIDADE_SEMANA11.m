...
% % PERCEPTRON POCKET PARA ANALISAR OS DADOS
[~,iPocket,erroPocket] = POCKET(xClasse1,xClasse2,10000,0.05);
%erroPocket=0 com finitas iterações do Perceptron Pocket! Isso significa
%que o algoritmo convergiu, isto é, as classes são linearmente separáveis.

% % % TESTE ESTATÍSTICO PARA REDUÇÃO DE DIMENSIONALIDADE
% [~,~,irrelevantes,~] = semana4_testeestatistico(xClasse1',xClasse2');
% 
...
% j = 1;
% k = 0;
% for i=1:size(padroes_norm,2)-1
%     if i~=irrelevantes(1,j) && j<=numel(irrelevantes)
%         k = k+1;
...
% end
% 
% relevantes_c1 = zeros(size(xClasse1,1),size(relevantes,2));
% relevantes_c2 = zeros(size(xClasse2,1),size(relevantes,2));
% j = 0;
% k = 0;
% for i=1:size(Classes,1)
%     if Classes(i)
...
% % % VERIFICANDO A EFICÁCIA DA REDUÇÃO DE DIMENSIONALIDADE COM PERCEPTRON
% [~,iPocket,erroPocket] = POCKET(relevantes_c1,relevantes_c2,10000,0.05);
% %algoritmo continua convergindo mesmo com redução de características. Então
% %os testes estatísticos agiram com sucesso e não eliminaram características 
% %importantes para a classificação (linear).

wh = waitbar(0,'PCA...');
i = 50;
while (i>1)
    [~,~,matrizmN,erro] = KL(padroes_norm',i);
    if erro>=0.01
        break;
    else
        i = i-1;
...
% % SEPARANDO PADRÕES DO NOVO ESPAÇO EM CLASSES
matrizmN = matrizmN';
j = 0;
k = 0;
for i=1:size(Classes,1)
    if Classes(i)
        j = j+1;
        pcaclasse1(j,:) = matrizmN(i,:);
    else
        k = k+1;
        pcaclasse2(k,:) = matrizmN(i,:);
    end
end

% % VERIFICANDO A EFICÁCIA DA REDUÇÃO DE DIMENSIONALIDADE COM PERCEPTRON
[~,iPocket,erroPocket] = POCKET(pcaclasse1,pcaclasse2,10000,0.05);
%perceptron pocket continua convergindo no espaço da PCA com 67 dimensões.

% % % % CALCULANDO AUC PARA AS CARACTERÍSTICAS
% for i=1:size(matrizmN,2)
%     AUCpca(i) = ROC(matrizmN(:,i),Classes);
% end
% maxAUCpca = find(AUCpca==max(AUCpca));
% 
% % % SELEÇÃO ESCALAR COM AUC (PESO 1)
% sqa1AUCpca = matrizmN(:,maxAUCpca).^2;
% sumsqa1AUCpca = sum(sqa1AUCpca);
% for i=1:size(matrizmN,2)
%     if i~=maxAUCpca
%         multAUCpca(:,i) = matrizmN(:,maxAUCpca).*matrizmN(:,i);
%         summultAUCpca = sum(multAUCpca(:,i));
%         sqajAUCpca(:,i) = matrizmN(:,i).^2;
%         sumsqajAUCpca = sum(sqajAUCpca(:,i));
%         sqrtsumsqajAUCpca = sqrt(sumsqa1AUCpca*sumsqajAUCpca);
%     	indicecAUCpca = summultAUCpca/sqrtsumsqajAUCpca;
%         mkAUCpca(i) = AUCpca(i) - 1*abs(indicecAUCpca);
%     end
% end
% maxmkAUCpca = find(mkAUCpca==max(mkAUCpca));
% ordempca = [maxAUCpca maxmkAUCpca];
% 
% % % PERCEPTRON POCKET COM AS CARACTERÍSTICAS SELECIONADAS
% classeApca = [matrizmN(Classes>0,ordempca(1)) matrizmN(Classes>0,ordempca(2))];
% classeBpca = [matrizmN(Classes<1,ordempca(1)) matrizmN(Classes<1,ordempca(2))];
% wp = POCKET(classeApca,classeBpca,10000,0.05);



%% 1) TREINANDO CLASSIFICADOR (ANTIGO)
...
% 
% %   SELEÇÃO VETORIAL
% % ordem = semana4_SelecaoVetorial('exaustivo','J3',classes,2);
% 
...
% 
% 
% %% 2) CLASSIFICANDO SEGUNDO CONJUNTO DE DADOS (ANTIGO)
% 
...
%     if i<numel(irrelevantes) && i~=irrelevantes(1,j)
%         k = k+1;
%         relevantes(:,k) = padroes_norm(:,i);
%     else
%         j = j+1;
