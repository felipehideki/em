function [h,p,irrelevantes,normais]=semana4_testeestatistico(classe1, classe2)
...
%NECESSITA DO TOOLBOX DE ESTATISTICA!
% pkg load statistics;
...
for i=1:L 
    n=0;
    [Hn]=swtest(classe1(i,:)); %testando normalidade da primeira classe
    if ~Hn
        [Hn]=swtest(classe2(i,:)); %testando normalidade da segunda classe
        if ~Hn 
            n=1; %se nao e possivel rejeitar a hipotese de normalidade
        else
    ...
    if n  %se nao e possivel rejeitar a hipotese de normalidade: ttest
        [h(i),p(i)]=ttest2(classe1(i,:),classe2(i,:));
        normais=[normais,i];
    else  %se a hipotese de normalidade foi rejeitada: ranksum
        fprintf('Chamando RankSum para caracteristica %d.\n',i) 
        [p(i),h(i)]=ranksum(classe1(i,:),classe2(i,:));        
%         [p(i),h(i)]=u_test(classe1(i,:),classe2(i,:));        
    end
end
irrelevantes=find(h==0);%nao significativas na distincao de classes
