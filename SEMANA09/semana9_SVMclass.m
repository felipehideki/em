function [classificacao,erro]=semana9_SVMclass(coefs,svms,kernel,kpar1,kpar2,w0,X,classesX)
...
%- kernel = metodo de kernel usado na funcao semana9_SVM
%- kpar1, kpar2 = parametros dos kernels usados na funcao semana9_SVM
...
for i=1:size(X,2)
    t=sum(coefs.*semana9_CalcKernel(svms',X(:,i)',kernel,kpar1,kpar2))+w0;
    ...
    for x1 = 1 : size(x,1)
        for y1 = 1 : size(x,2)
            ...
            for i = 1 : length(coefs)
                    z(x1,y1) = z(x1,y1) + coefs(i)*semana9_CalcKernel(ponto,svms(:,i)',kernel,kpar1,kpar2);
            end
        end        
        waitbar((x1)/size(x,1)) ;
        drawnow
